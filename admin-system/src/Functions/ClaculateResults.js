 ...
 */
export function calculateElectedSenators(num_vacancies, all_candidates, all_votes)
{
    ...
    while( num_vacancies > 0 )
    {
        ...
        if(max_vote_count >= quota)
        {
            ...
            first_pref_votes = all_candidates[max_cand_id].first_pref_votes;
        ...
        else
        {
            while(max_vote_count < quota)
            {
                ...
                first_pref_votes = all_candidates[min_cand_id].first_pref_votes;
                these_votes_size = first_pref_votes.length;
                transfer_votes = [];
                for(i = 0; i < num_candidates; i++)
                {
